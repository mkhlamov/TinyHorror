...
public class OVRControllerHelper : MonoBehaviour
{
    ...
    /// <summary>
	/// The root GameObject that represents the gun with a flashlight.
	/// </summary>
    public GameObject m_modelGun;
	...
	void Update()
	{
        ...
        if ((controllerConnected != m_prevControllerConnected) || !m_prevControllerConnectedCached)
		{
            if (m_modelGun != null)
            {
                m_modelGun.SetActive(controllerConnected);
            ...
            } else if (activeControllerType == ControllerType.GearVR || activeControllerType == ControllerType.Go)
			{
